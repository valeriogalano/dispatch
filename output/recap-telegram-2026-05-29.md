_Questo testo è stato generato con gemini-3-flash-preview_

🛠 **Novità tra quiz AI e time tracking**

Valerio ha dedicato la settimana a potenziare gli strumenti di automazione e a rifinire le funzionalità core delle sue applicazioni desktop e mobile, con un focus particolare sulla gestione del tempo.

🤖 **Podcast Quiz to Telegram**
* **Modernizzazione API**: Valerio ha aggiornato il bot alla versione 9.0 della Bot API di Telegram, introducendo il supporto nativo alle risposte multiple e spostando lo snippet del quiz nel nuovo campo `description`.
* **Multi-provider AI**: Aggiunto il supporto ad Anthropic (Claude Haiku 4.5) in affiancamento a Google Gemini. Ora il modello utilizzato per generare il quiz viene mostrato esplicitamente agli utenti.
* **Robustezza**: Migliorata la gestione dei limiti di caratteri per le spiegazioni e risolti bug relativi al troncamento del JSON in output.
* **GitHub**: https://github.com/valeriogalano/podcast-quiz-to-telegram

⏱️ **Timebox**
* **Ore Fatturabili**: Introdotta la distinzione tra ore tracciate e ore effettivamente fatturabili. La funzionalità include un toggle di vista nella dashboard, supporto nel CLI e nei tool MCP, e indicatori visivi per le divergenze.
* **QuickLog**: Aggiunta la sintassi inline `+Nh` (es. `+1h30m`) nella palette di ricerca per sommare ore direttamente a un progetto senza cambiare vista.
* **Interfaccia**: Nuovi shortcut da tastiera (`?` per l'aiuto, `⌘⇧V` per il cambio vista, `⌘⇧H` per nascondere i progetti vuoti) e migliorata la visualizzazione dei task Todoist.

📱 **KeepInTouch & Highlighter**
* **Soglie Contatti**: KeepInTouch permette ora di sovrascrivere le soglie di notifica globali per singoli contatti, salvando i dati direttamente nei metadati della rubrica iOS.
* **Export Picker**: In Highlighter, l'opzione di condivisione tramite il foglio di sistema è ora sempre disponibile tra le scelte di esportazione.

🔧 **Infrastruttura e Assets**
* **Dev Updates**: Migliorato lo script di generazione di questi recap, aggiungendo il supporto a link personalizzati e l'oscuramento automatico dei link a repository privati.
* **Codeberg**: Gli asset di Pensieri in codice segnalano ora il passaggio ufficiale a Codeberg come mirror canonico.

📖 Articolo completo: https://pensieriincodice.it/blog/2026-05-29-recap/
#recap
